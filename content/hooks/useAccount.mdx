...
```tsx
import { useAccount } from "@dogeos/dogeos-sdk";
import type { SwitchChainOptions } from "@dogeos/dogeos-sdk";
...
Read live balances through `currentProvider`. Use `eth_getBalance` for EVM wallets and `getBalance()` for Dogecoin wallets. See [Read Balances](/hooks/useAccount/balance) for examples.
