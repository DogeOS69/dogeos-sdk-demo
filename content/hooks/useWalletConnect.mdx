...
```tsx
import { useWalletConnect } from "@dogeos/dogeos-sdk";
import type { ConnectVariables } from "@dogeos/dogeos-sdk";
...
Error message if an error occurred during connection.
...
Boolean indicating whether no wallet is currently connected.

**Type:** `boolean`

### walletStatus

Current embedded wallet readiness state.

**Type:** `"disabled" | "initializing" | "ready" | "unavailable" | "error"`

### isWalletReady

Boolean indicating whether the embedded wallet is ready for use.

**Type:** `boolean`

### isWalletLoading

Boolean indicating whether the embedded wallet is still initializing.
...
export function WalletConnection() {
  ...
  return (
    <div>
      ...
      <div>Ready: {isWalletReady ? "Yes" : "No"}</div>
      {isConnected ? (
        <button onClick={disconnect}>Disconnect</button>
      ) : (
